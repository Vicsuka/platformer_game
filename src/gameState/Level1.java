...
public class Level1 extends GameState{
			...
			// sarkany repulese
			private int flyingDir = 1;
			private int flyingSpeed;
			private int flyingInc = 2;
			...
			public void levelseq() {
				...
				if (canSpawnEnemy) {
					...
					if (enemySpawn == 27) {
						...
						flyingSpeed = 0;
						int enemyX = 320;
						int enemyY = ThreadLocalRandom.current().nextInt(120,200 + 1);
						enemy = new Enemy();
					...
					// random hogy eppen tamad-e
					int doAttack = ThreadLocalRandom.current().nextInt(1, 150 + 1);
					if (doAttack == 27) {
					...
					if (flyingSpeed == flyingInc) {
						enemy.setPosition(enemy.getX()-1, enemy.getY()+flyingDir);
						flyingSpeed = 0;
					}
			...
			public void update() {
				...
				if (toKill == 0) {
					...
					if (timer > 0) {
						...
						gsm.player.setLevel(GameStateManager.LEVEL2);
			...
			public void draw(java.awt.Graphics2D g) {
				...
				if (enemy != null) {
					g.setFont(font);
					g.setColor(color);
					g.drawString("Enemy HP", 240, 10);
					g.setColor(Color.BLUE);
					g.fillRect(210, 12, 10*enemy.getHP(), 5);
				}
