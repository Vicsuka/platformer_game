...
public class Level3 extends GameState{
			...
			public Level3(GameStateManager gsm){
				...
				try {
					bg = new Background("/Backgrounds/level3.png",1);
					bg.setVector(0,0);
			...
			public void levelseq() {
				...
				if (canSpawnEnemy) {
					int enemySpawn = ThreadLocalRandom.current().nextInt(1, 2 + 1);
					if (enemySpawn == 2) {
						canSpawnEnemy = false;
						int enemyX = 320;
						int enemyY = player.getY();
						enemy = new Enemy();
					...
					//koveti a jatekos poziciojat
					if (enemy.getY() > player.getY()) {
						enemy.setPosition(enemy.getX()-1, enemy.getY()-1);
			...
			public void update() {
				...
				if (toKill == 0) {
					...
					if (timer > 0) {
						...
						gsm.player.setLevel(GameStateManager.LEVEL4);
			...
			public void draw(java.awt.Graphics2D g) {
				...
				for (int i = 0; i < projectiles.size(); i++) {
					...
					if (projectiles.get(i).getX() - player.getX() > 120) {
